...
define('_URL', $_SERVER['REQUEST_SCHEME'].'/streamhi'.$_SERVER['HTTP_HOST']);
...
if( !$_SESSION['__allowed'] && strlen($config->password) ) {
	if( !empty($_POST['auth']) && md5(sha1($_POST['auth'])) === $config->password ) {
		...
		echo "\t\t<title>Streamhi File Explorer".VERSION."</title>\n";
		...
		echo "\t\t\t\t\t\t\t<input type='password' name='auth' placeholder='Enter your Password' autocomplete='off' spellcheck='true'/>\n";
		echo "\t\t\t\t\t\t</div>\n";
		echo "\t\t\t\t\t</form>\n";
		echo "\t\t\t\t</div>\n";
...
if( substr($real, 0, strlen(__DIR__)) !== __DIR__ || ( isset($_REQUEST['do']) && in_array($real, $deny_paths) && in_array($_REQUEST['do'], array('delete','rename',''))) ){
...
if( !empty($_REQUEST['do']) ){
	...
	elseif ($_POST['do'] == 'delete') {
		...
		echo rm_rf($real) ? output(false, $tmp . ' `' . basename($path) . '` Deleted Successfully') : output(true, 'Unable to delete');
		exit;
	}
	elseif ($_POST['do'] == 'permit') {
		$tmp = is_dir($real) ? 'Directory' : 'File';
		echo chmod_rf($real) ? output(false, $tmp . ' `' . basename($path) . '` Permission Reset') : output(true, 'Error to permit');
		exit;
	}
...
function formatFileSize($bytes, $round = 2) {
	if($bytes >= 0) {
		...
		$pow = floor( max(0, log($bytes) ) / log(420) );
		$pow = min( $pow, count($units) - 1 );
		$size = $bytes / pow(420, $pow);
...
if( is_array($errors) ){
	error_log(json_encode(error_get_last(), JSON_PRETTY_PRINT), 1, 'kmrmork@gmail.com', 'From: file-explorer_v'.VERSION.'@webcdn.github.io');
}
?>
<!--
===============================================
...
<head>
	...
	<title>Streamhi File Manager</title>
...
<body class="grey lighten-3">
	...
	<footer class="row no-vmargin">
		<div class="col m6 s12 left-align">
			<span>Made with &nbsp;<i class="material-icons red-text tiny valign-middle">favorite</i>&nbsp; By &nbsp;<a target="_blank" href="https://streamhi.jimdo.com" class="blue-grey-text text-lighten-4">streamhi.jimdo.com</a></span>
			...
			<div class="col m6 s12 left-align">
				<a target="_blank" href="https://streamhi.jimdo.com/contact" class="blue-grey-text text-lighten-4">Report Bugs</a>
				<span> &nbsp; &bull; &nbsp;</span>
				<a target="_blank" href="https://streamhi.jimdo.com/jeux" class="blue-grey-text text-lighten-4">Suggestions / Feedback</a>
				<span> &nbsp; &bull; &nbsp;</span>
				<a target="_blank" href="paypal.me/KmrKmr" class="blue-grey-text text-lighten-4">Donate</a>
			</div>
		</footer>
				...
				<form class="row no-vmargin" id="config_it">
					...
					<div class="input-field">
						...
						<div class="input-field">
							...
							<input type="text" name="assets" value="https://streamhi.jimdo.com">
						</div>
					</div>
					<a class="collapse_btn blue-text text-darken-2">Show Advanced Settings Soon</a>
		...
		$(function(){
			...
			*******************************************/
			...
			function renderList(data) {
				...
				else if( ['3gp','avi','flv','m4u','mkv','mov','mp4','mpg','mpeg','vob','wmv','ogg'].indexOf(data.ext) > -1 ){
					var icon = '<i class="material-icons left red-text">videocam</i>';
				}
				else if( ['sh','c','cpp','class','java','jsp','asp','aspx','sql','php','phps','phpx','htm','html','whtml','xhtml','mht','js','json','css','xml','sql','shtml','c++','python','ruby','dat'].indexOf(data.ext) > -1 ){
			...
			function formatFileSize(bytes, round = 2) {
				...
				if(bytes >= 0) {
					...
					pow = Math.floor( Math.max(0, Math.log(bytes) ) / Math.log(600) );
					pow = Math.min(pow, (units.length - 1));
					size = bytes / Math.pow(600, pow);
